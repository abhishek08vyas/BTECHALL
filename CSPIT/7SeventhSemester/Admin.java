...
public class Admin extends Login {
	...
	public boolean AddReportingAuthority() {
		...
		System.out.println("Please enter employee under RA(" + sUserId
				+ ")\n\tnote: more than one employee userid should be separated by comma!!");
		...
		for (int i = 0; i < arrUsers.length; i++) {
			if (IsUseridUnique(arrUsers[i], EnLoginUserType.Employee.toString()) == false) {
				System.out.println(arrUsers[i] + " is already assigned in another RA!!");
				arrUsers[i] = null;
			}
		}
		...
		if (lstEmpInfo.size() == 0) {
			if (arrUsers[0] != null) {
				System.out.println("Currently employee " + arrUsers[0] + " not available.");
			...
			for (int j = 0; j < arrUsers.length; j++) {
				for (int i = 0; i < lstEmpInfo.size(); i++) {
					if (lstEmpInfo.get(i).GetUserId() != null && arrUsers[j] != null
						...
						System.out.println("Currently employee " + arrUsers[j] + " not available.");
						String sEmpPassword = getValidPassword();
						TempEmp = new Employee();
						TempEmp.SetLeave(nTotalLeave);
	...
	private boolean AddEmployee() {
		...
		System.out.println("Please provide RA for " + sUserId);
		String sRA = getValidUserId("");
		if (IsUseridUnique(sRA, EnLoginUserType.RA.toString()) == true) {
			System.out.println("Currently RA " + sRA + " not available.");
